...
Visibility is the account holder's decision, taken on the node. An agent
deciding its own reachability would let one tenant's compromised agent open
...
* visible (bool, required) – True opens the agent, false closes it.
...
```shellsession
$ astral-query mcp.set_visible -id scout -visible true -out json
{"Type":"ack","Object":null}
```

```shellsession
$ astral-query mcp.set_visible -id scout -visible false -out json
{"Type":"ack","Object":null}
```
