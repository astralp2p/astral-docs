...
Permanently ban the named node from the swarm and return the signed expulsion. The caller must hold `mod.user.admin_swarm_action`; the active contract's issuer holds it by default. Rejected with code `2` if the node has no active contract, with code `3` if `identity` does not resolve, and with code `4` if the caller is not authorized. The ban is identity-level and irreversible.
