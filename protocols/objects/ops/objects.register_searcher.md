...
Register the caller as an external searcher. The module proxies `objects.search` calls back to the caller's identity until the registration's lease expires. Network-originated queries are rejected.
...
adding a second one, so a searcher renews by registering again. Expiry is
silent: an expired registration receives no further search calls and the node
...
* A `mod.objects.registration_lease` object once the external searcher is registered, carrying the lease the node granted.
...
```shellsession
$ astral-query objects.register_searcher -duration 24h -out json
