...
Register the caller as an external finder. The module proxies `objects.find` calls back to the caller's identity until the registration's lease expires. Network-originated queries are rejected.
...
adding a second one, so a finder renews by registering again. Expiry is silent:
an expired registration receives no further find calls and the node sends the
registrant no notice.
...
* A `mod.objects.registration_lease` object once the external finder is registered, carrying the lease the node granted.
...
```shellsession
$ astral-query objects.register_finder -duration 24h -out json
