...
a body left out for room is marked `Truncated`.
...
* An `error_message` object if the input is not one
  `messaging.read_messages_request`, or the rows cannot be read.
